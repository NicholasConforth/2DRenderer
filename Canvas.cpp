...
void Canvas::DrawLineList(const std::vector<SDL_Point>& points, const SDL_Color& color)
{
    ...
    {
        SDL_Point prevPoint = points[i];
        if (i != 0)
        {
            DrawLine(prevPoint, points[i], color);
        }
    }
}
...
void Canvas::DrawTri(const SDL_Point& point1, const SDL_Point& point2, const SDL_Point& point3, const SDL_Color& color)
{
    DrawPoint(point1, color);
    DrawPoint(point2, color);
    DrawPoint(point3, color);
...
void Canvas::DrawImage(const Image* image, const SDL_Point& point)
{
    for (int y = 0; y < image->m_height; y++)
    {
        ...
        for (int x = 0; x < image->m_width; x++)
        {
            ...
            // color = (source color * sorce alpha) + (destiantion color * (1 - sorce alpha))
...
void Canvas::ProcessThreshold(const SDL_Color& threshold)
{
    for (pixel_t& pixel : m_buffer)
    {
        ...
        color.g = (color.g >= threshold.g) ? color.r : 0;
        color.b = (color.b >= threshold.b) ? color.r : 0;
